...
parser.add_argument(
    "--data_path",
    default="./kitti/frustum_all_raw/2011_09_26_0001.pickle",
...
def write_detection_results(
    ...
    for idx in results:
        ...
        if kitti_type == "raw":
            pred_filename = os.path.join(output_dir, f"{str(idx).zfill(10)}.txt")
...
def test_from_rgb_detection(
    output_filename,
    result_dir=None,
    kitti_type=None,
    frustum_dataset: provider.FrustumDataset = None,
):
    ...
    test_idxs = np.arange(0, len(frustum_dataset))
    print(len(frustum_dataset))
    batch_size = BATCH_SIZE
    num_batches = int((len(frustum_dataset) + batch_size - 1) / batch_size)
    ...
    for batch_idx in tqdm(range(num_batches)):
        ...
        end_idx = min(len(frustum_dataset), (batch_idx + 1) * batch_size)
    ...
    write_detection_results(
        result_dir,
        frustum_dataset.id_list,
        frustum_dataset.type_list,
        frustum_dataset.box2d_list,
        center_list,
        heading_cls_list,
        heading_res_list,
        size_cls_list,
