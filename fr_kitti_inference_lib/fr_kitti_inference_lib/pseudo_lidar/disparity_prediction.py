...
parser.add_argument(
    "--loadmodel", default="./log/pretrained_model_KITTI2015.tar", help="loading model"
)
...
def predict_disparity(test_left_img, test_right_img):

    normal_mean_var = {"mean": [0.485, 0.456, 0.406], "std": [0.229, 0.224, 0.225]}
    infer_transform = transforms.Compose(
        [transforms.ToTensor(), transforms.Normalize(**normal_mean_var)]
    )
