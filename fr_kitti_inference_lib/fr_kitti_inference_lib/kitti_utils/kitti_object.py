...
# from kitti.viz_util import draw_lidar_simple, draw_lidar, draw_gt_boxes3d
...
class kitti_object(object):
    ...
    def __init__(self, root_dir, split="training"):
        ...
        self.pseudo_lidar_dir = os.path.join(self.split_dir, "pseudo-lidar_velodyne")
    ...
    def get_label_objects_predictions(self, idx, type="lidar"):
        # assert(idx<self.num_samples and self.split=='training')
        base_dir = (
            "/home/vassili/projects/Object_Detection/frustum-pointnets/train/detection_results_v1_"
            + type
            + "/data"
        )
        label_filename = os.path.join(base_dir, "%06d.txt" % (idx))
        return utils.read_label(label_filename)
...
def dataset_viz():
    ...
    for data_idx in range(1, len(dataset)):
        ...
        try:
            objects_lidar = dataset.get_label_objects_predictions(
                data_idx, type="lidar"
            )
            objects_pseudo_lidar = dataset.get_label_objects_predictions(
                data_idx, type="pseudo_lidar"
            )
        except:
            show_image_with_boxes(
                img, objects_gt, calib, True, box_color=(255, 255, 255), thickness=3
            )
            meow = 1
            continue

        # objects = dataset.get_label_objects(data_idx)
        objects_lidar[0].print_object()
        ...
        img = show_image_with_boxes(
            img, objects_gt, calib, False, box_color=(255, 255, 255), thickness=3
        )
        img = show_image_with_boxes(
            img, objects_lidar, calib, False, box_color=(255, 0, 0), thickness=2
        )
        img = show_image_with_boxes(
            img, objects_pseudo_lidar, calib, True, box_color=(0, 255, 0), thickness=1
        )
        ...
        cv2.waitKey(0) & 0xFF
        # except:
        #     objects = dataset.get_label_objects_predictions(data_idx)
        ...
        #     # Draw 2d and 3d boxes on image
        #     show_image_with_boxes(img, [], calib, True)
        #     raw_input()
