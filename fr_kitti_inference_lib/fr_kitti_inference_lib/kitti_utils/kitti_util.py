...
class CalibrationRaw(Calibration):
    def __init__(self, dataset: pykitti.raw):
        ...
        calibs = dataset.calib
        self.P = calibs.P_rect_20
        # self.P_rect = np.reshape(calibs.P_rect_20, [3, 4])
        self.P = np.reshape(self.P, [3, 4])
        # Rigid transform from Velodyne coord to reference camera coord
        # self.V2C = calibs["Tr_velo_to_cam"]
        # self.V2C = calibs.T_cam0_velo[:3, :]

        self.V2C = calibs.T_cam2_velo[:3, :]

        # self.V2C = np.reshape(self.V2C, [3, 4])
        self.C2V = inverse_rigid_trans(self.V2C)
        # Rotation from reference camera coord to rect camera coord
        # self.R0 = calibs.R_rect_00
        # self.R0 = calibs.R_rect_00[:3, :3]

        self.R0 = calibs.R_rect_20[:3, :3]
        # self.R0 = np.reshape(self.R0, [3, 3])
